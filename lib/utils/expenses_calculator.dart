...
import 'package:ai_chat/utils/analytics.dart';
...
class ExpensesCalculator {
  ...
  ///
  /// Очищает текущий кэш и заполняет его новыми данными о моделях.
  /// Используется для обновления цен моделей при изменении списка доступных моделей.
  ///
  /// Параметры:
  ...
  /// Вычисляет номер недели для указанной даты.
  ///
  /// Использует алгоритм ISO 8601 для определения недели года.
  /// Неделя начинается с понедельника. Неделя 1 - это первая неделя года,
  /// содержащая минимум 4 дня.
  ///
  /// Параметры:
  /// - [date]: Дата для вычисления номера недели.
  ///
